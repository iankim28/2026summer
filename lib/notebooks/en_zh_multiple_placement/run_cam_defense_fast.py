...
def build_attacked_images(base_imgs, img_indices, attack_lang, target, n_workers=None):
    ...
    with ThreadPoolExecutor(max_workers=n_workers) as pool:
        return list(pool.map(
            lambda args: draw_word(args[0], args[1], img_idx=args[2], already_224=True),
            zip(base_imgs, words, img_indices),
        ))
...
def load_cam_cache(condition, n):
    path = os.path.join(CACHE_DIR, f'cams_{condition}_dual{NUM_BOXES}_fs{FONT_SIZE}_n{n}.npz')
