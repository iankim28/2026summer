"""Japanese CLIP Defense-Prefix eval on PROTOCOL dual-box CIFAR.
...
from ja_dp_encode import (  # noqa: E402
    JA_MODEL_ID,
    class_prompts,
    encode_images,
    encode_text_vanilla,
...
CIFAR_TOKEN_JA = RESULTS / "dp_cifar10_ja_vit-b16.pt"
COST_PROXY = 2
...
@torch.no_grad()
def classify(model, preprocess, imgs, text_feat, batch_size=64):
    preds = []
    for i in range(0, len(imgs), batch_size):
        batch = imgs[i : i + batch_size]
        pv = preprocess_images(preprocess, batch, DEVICE)
...
def run_eval(n: int, status: str, token_path: Path):
    ...
    vanilla_txt = encode_text_vanilla(model, tokenizer, prompts_v, DEVICE)
    dp_txt = encode_text_with_dp(model, tokenizer, prompts_dp, dp_vec, DEVICE)
    cos = float((vanilla_txt * dp_txt).sum(-1).mean().item())
    print(f"Mean cosine(vanilla, DP JA text feats) = {cos:.4f} (must be < 1)")
    if cos > 0.9999:
        raise RuntimeError("JA DP token had no effect on text features")

    attacked, _ = build_multi_attack(data)
    true, target = data["true"], data["target"]

    clean_vanilla = classify(model, preprocess, data["clean_224"], vanilla_txt)
    atk_vanilla = classify(model, preprocess, attacked, vanilla_txt)
    clean_acc = float((clean_vanilla == true).mean())
    atk_acc = float((atk_vanilla == true).mean())
    atk_asr = float((atk_vanilla == target).mean())
    print(
        f"Vanilla JA clean={100*clean_acc:.1f}%  attack_acc={100*atk_acc:.1f}%  "
    ...
    for i in range(len(attacked)):
        pred = classify(model, preprocess, [attacked[i]], dp_txt)[0]
    ...
    clean_dp = classify(model, preprocess, data["clean_224"], dp_txt)
    clean_dp_acc = float((clean_dp == true).mean())
    clean_delta = clean_dp_acc - clean_acc

    changed = int((def_preds != atk_vanilla).sum())
    print(
        f"JA DP defended_acc={100*def_acc:.1f}%  ASR={100*def_asr:.1f}%  "
    ...
    if status == "sanity" and changed == 0 and abs(def_acc - atk_acc) < 1e-9:
        raise RuntimeError("Gate A fail: JA DP predictions identical to no-defense")
    # Smoke/final still write even if weak — Table 1 needs a number or documented fail.
    ...
    payload = {
        ...
        "defense_acc_ja": def_acc,
        "clean_delta_ja": clean_delta,
        "preds_changed_vs_vanilla": changed,
        "mean_cosine_vanilla_dp_text": cos,
        "notes": (
            "CIFAR-trained llm-jp CLIP DP token; JA prompts "
            "(*{}の写真。); multi EN+ZH stickers at eval; EN+JA multi train."
        ),
    }
    out = RESULTS / f"comparison_summary_{status}_n{data['n']}_ja.json"
    write_summary(out, payload)
    write_summary(RESULTS / "comparison_summary_ja.json", payload)
    return payload
...
def main():
    ...
    args = ap.parse_args()
    token = Path(args.token) if args.token else CIFAR_TOKEN_JA
    if not token.is_file():
        raise FileNotFoundError(f"JA DP token not found: {token}")
