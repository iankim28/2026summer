"""Korean CLIP Defense-Prefix eval on PROTOCOL dual-box CIFAR.
...
from ko_dp_encode import (  # noqa: E402
    KO_MODEL_ID,
    class_prompts,
    encode_images,
    encode_text_vanilla,
...
CIFAR_TOKEN_KO = RESULTS / "dp_cifar10_ko_vit-b32.pt"
COST_PROXY = 2
...
def run_eval(n: int, status: str, token_path: Path):
    ...
    model, processor = load_ko_clip(DEVICE)
    ...
    print(f"Mean cosine(vanilla, DP KO text feats) = {cos:.4f} (must be < 1)")
    if cos > 0.9999:
        raise RuntimeError("KO DP token had no effect on text features")
    ...
    print(
        f"Vanilla KO clean={100*clean_acc:.1f}%  attack_acc={100*atk_acc:.1f}%  "
    ...
    print(
        f"KO DP defended_acc={100*def_acc:.1f}%  ASR={100*def_asr:.1f}%  "
    ...
    if status == "sanity" and changed == 0 and abs(def_acc - atk_acc) < 1e-9:
        raise RuntimeError("Gate A fail: KO DP predictions identical to no-defense")
    ...
    payload = {
        ...
        "notes": (
            "CIFAR-trained Korean CLIP DP token; KO prompts "
            "(*{}의 사진.); multi EN+ZH stickers at eval; EN+KO multi train."
        ),
    }
    out = RESULTS / f"comparison_summary_{status}_n{data['n']}_ko.json"
    write_summary(out, payload)
    write_summary(RESULTS / "comparison_summary_ko.json", payload)
    return payload
...
def main():
    ...
    args = ap.parse_args()
    token = Path(args.token) if args.token else CIFAR_TOKEN_KO
    if not token.is_file():
        raise FileNotFoundError(f"KO DP token not found: {token}")
