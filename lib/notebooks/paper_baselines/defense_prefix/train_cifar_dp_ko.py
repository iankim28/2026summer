"""Train Korean CLIP Defense-Prefix on CIFAR-10 train (never uses eval sample)."""
...
TOKEN_OUT = OUT_DIR / "dp_cifar10_ko_vit-b32.pt"
...
def _ko_font_path():
    if platform.system() == "Windows":
        return os.path.join(os.environ.get("WINDIR", r"C:\Windows"), "Fonts", "malgun.ttf")
    fp = "/usr/share/fonts/truetype/noto/NotoSansCJK-Regular.ttc"
    if not os.path.isfile(fp):
        fp = "/usr/share/fonts/opentype/noto/NotoSansCJKkr-Regular.otf"
    return fp
...
FONT_KO = _font(_ko_font_path())
FONT_LAT = _font(_lat_font_path())


def draw_typo_ko(img: Image.Image, wrong: int, rng: random.Random) -> Image.Image:
    """EN+KO dual-box attack matching ZH multi-train distribution."""
    img = img.convert("RGB").resize((DISPLAY, DISPLAY), Image.BICUBIC)
    draw = ImageDraw.Draw(img)
    placed = []
    for word, font in ((EN_CLASSES[wrong], FONT_LAT), (KO_CLASSES[wrong], FONT_KO)):
...
class CifarTypoTrainKo(Dataset):
    ...
    def __getitem__(self, i):
        ...
        typo = draw_typo_ko(img, wrong, rng)
...
def train(epochs=10, batch_size=128, lr=0.002, max_n=20000, gamma=3.0, seta=1.0):
    ...
    ds = CifarTypoTrainKo(seed=0, max_n=max_n)
    loader = DataLoader(
        ds,
    ...
    print(
        f"KO DP train samples={len(ds)} batch={batch_size} epochs={epochs} "
        f"device={device} attack=EN+KO_multi",
        flush=True,
    )
    ...
    for ep in range(epochs):
        ...
        torch.save(emb.weight.detach().cpu(), OUT_DIR / f"dp_cifar10_ko_ep{ep+1}.pt")
