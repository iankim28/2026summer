"""SamplingTAR hybrid on Korean HF CLIP (Bingsu ViT-B/32)."""
...
def tune_z(ko, text_emb, clean, attacked, true, means, mu, sigma, probe_n=40):
    ...
    for z in [2.0, 1.5, 1.0, 0.5]:
        ...
        heads = heads[:16]
        c_acc = float(
            (classify_with_heads_hf(ko, clean_p, text_emb, heads) == true_p).mean()
        )
        a_acc = float(
            (classify_with_heads_hf(ko, atk_p, text_emb, heads) == true_p).mean()
        )
...
def run_eval(n: int, status: str, top_k_patches=DEFAULT_TOP_K, score_frac=DEFAULT_SCORE_FRAC):
    ...
    if status == "sanity" and max(means.values()) <= 0:
        raise RuntimeError("Gate A fail: KO head mining produced zero scores")

    heads, z_used, thr = tune_z(
        ko, text_emb, data["clean_224"], attacked, true, means, mu, sigma,
    ...
    with open(RESULTS / "selected_heads_ko.json", "w", encoding="utf-8") as f:
        json.dump({"lang": "ko", "heads": heads, "z": z_used, "thr": thr, "grid": GRID}, f, indent=2)
    ...
    if status == "sanity" and changed == 0 and len(heads) > 0:
        raise RuntimeError("Gate A fail: KO SamplingTAR hybrid did not change predictions")
    clean_def_acc = float((clean_def == true).mean())
    clean_delta = clean_def_acc - clean_acc
    print(
        f"SamplingTAR [hybrid] KO acc={100*def_acc:.1f}% Clean_delta={100*clean_delta:.1f}pp "
    ...
    payload = {
        ...
        "notes": "SamplingTAR hybrid on HF KO CLIP; re-mined heads; attn-guided blur.",
    }
