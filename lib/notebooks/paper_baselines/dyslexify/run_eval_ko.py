"""Dyslexify hybrid on Korean HF CLIP (Bingsu ViT-B/32)."""
...
from _common.partner_models import CLASSES, KoCLIP  # noqa: E402
...
def greedy_select(ko, text_emb, clean, attacked, true, ranked,
                  eps=0.001, stop_at_delta=0.05, max_heads=24, probe_n=40):
    clean_p = clean[:probe_n]
    atk_p = attacked[:probe_n]
    true_p = true[:probe_n]
    base_clean = float((classify_batch(ko, clean_p, text_emb) == true_p).mean())
    base_atk = float((classify_batch(ko, atk_p, text_emb) == true_p).mean())
    ...
    for score, L, H in ranked:
        cand = selected + [(L, H)]
        c_acc = float((classify_with_heads_hf(ko, clean_p, text_emb, cand) == true_p).mean())
        a_acc = float((classify_with_heads_hf(ko, atk_p, text_emb, cand) == true_p).mean())
        gain = a_acc - cur_atk
...
def run_eval(n: int, status: str, top_k_patches=DEFAULT_TOP_K, score_frac=DEFAULT_SCORE_FRAC):
    ...
    ko = KoCLIP()
    text_emb = ko.embed_texts(CLASSES["ko"])

    mine_n = min(50, data["n"])
    ranked, _ = typographic_scores_hf(
        ko, attacked, rects, max_images=mine_n, n_layers=N_LAYERS, n_heads=N_HEADS, grid=GRID
    )
    if status == "sanity" and ranked[0][0] <= 0:
        raise RuntimeError("Gate A fail: KO typographic scores all zero")

    heads = greedy_select(
        ko, text_emb, data["clean_224"], attacked, true, ranked,
    ...
    with open(RESULTS / "selected_heads_ko.json", "w", encoding="utf-8") as f:
        json.dump({"lang": "ko", "heads": heads, "n_mine": mine_n, "grid": GRID}, f, indent=2)

    clean_p = classify_batch(ko, data["clean_224"], text_emb)
    atk_p = classify_batch(ko, attacked, text_emb)
    clean_acc = float((clean_p == true).mean())
    atk_acc, atk_asr = acc_asr(atk_p, true, target)
    print(f"Vanilla KO clean={100*clean_acc:.1f}% atk={100*atk_acc:.1f}% ASR={100*atk_asr:.1f}%")

    t0 = time.time()
    def_preds, mean_patches = classify_hybrid_hf(
        ko, attacked, text_emb, heads, top_k_patches=top_k_patches, score_frac=score_frac, grid=GRID
    )
    clean_def, _ = classify_hybrid_hf(
        ko, data["clean_224"], text_emb, heads, top_k_patches=top_k_patches, score_frac=score_frac, grid=GRID
    )
    ...
    if status == "sanity" and changed == 0 and len(heads) > 0:
        raise RuntimeError("Gate A fail: KO Dyslexify hybrid did not change predictions")
    clean_def_acc = float((clean_def == true).mean())
    clean_delta = clean_def_acc - clean_acc
    print(
        f"Dyslexify [hybrid] KO acc={100*def_acc:.1f}% Clean_delta={100*clean_delta:.1f}pp "
    ...
    payload = {
        ...
        "defense_acc_ko": def_acc,
        "clean_delta_ko": clean_delta,
        "preds_changed_vs_vanilla": changed,
        "notes": "Dyslexify hybrid on HF KO CLIP; re-mined heads; attn-guided blur.",
    }
    write_summary(RESULTS / f"comparison_summary_{status}_n{data['n']}_hybrid_ko.json", payload)
    write_summary(RESULTS / "comparison_summary_hybrid_ko.json", payload)
    return payload
