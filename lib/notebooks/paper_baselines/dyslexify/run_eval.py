...
CLS→spatial attention (alpha=1) at inference. open_clip ViT (EN B/32 or JA B/16).
...
@torch.no_grad()
def typographic_scores(en, attacked, rects, max_images=50, n_layers=12, n_heads=12, grid=7):
...
def run_eval(
    ...
    ranked, _score_mat = typographic_scores(
        model,
        attacked,
        rects,
        max_images=mine_n,
    ...
    with open(RESULTS / heads_name, "w", encoding="utf-8") as f:
        json.dump(
            {"lang": lang, "heads": heads, "n_mine": mine_n, "mode": mode, "grid": cfg["grid"]},
            f,
            indent=2,
        )
    ...
    if mode == "hybrid":
        ...
        cost = COST_HYBRID
        notes = (
            f"Dyslexify hybrid ({lang}): typo-attn heads + attn-guided blur "
            f"(top_k={top_k_patches}, score_frac={score_frac}, grid={cfg['grid']}) "
            "+ CLS attn redirect."
        )
        variant = "heads+attn_blur"
    else:
        def_preds = classify_with_heads(model, attacked, text_emb, heads, batch_size=32)
        clean_def = classify_with_heads(model, data["clean_224"], text_emb, heads)
        cost = COST_HEADS
        notes = (
            f"Dyslexify-style ({lang}): typo-attn score + greedy select; "
            "CLS attn redirect (alpha=1)."
        )
        variant = "heads"
    ...
    print(
        f"Dyslexify [{mode}] {lang.upper()} acc={100*def_acc:.1f}% "
        f"Clean_delta={100*clean_delta:.1f}pp heads={len(heads)}"
