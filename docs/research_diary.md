...
| Defense-Prefix (CIFAR-trained) | 73.8% EN | +0.5pp | 2 | EN-only |
...
Published ImageNet `dp_vit-b32.pt` failed Gate A (0/16 preds changed). Retrained 10 epochs on CIFAR-10 **train** (20k, synthetic dual-box typos; eval sample unused). Final: EN **73.8%**, Clean Δ **+0.5pp**, cost 2 — beats our EN 71.6% and is kinder on clean images, but residual ASR **16.4%** (ours ~2.6%) and no ZH DP token, so it does not replace EN∩ZH mean **74.9%**.
