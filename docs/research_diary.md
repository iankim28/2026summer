...
**Status:** Both re-runs complete on CUDA with frozen `attack_pos`. Free-tune numbers below;
**thr ≥ 0.95 floor applied next** — see 2026-07-21 entry.

---

## 2026-07-21 — Thr ≥ 0.95 floor restores new-protocol four_lang accuracy

**Fix:** After free thr-tune on n=100, four_lang now sets
`threshold = max(threshold_free, 0.95)` for the full n=1000 run. Logs both values.
Enforced in [`_cells/14_main.py`](../lib/notebooks/four_lang_cc_bbox_blur/_cells/14_main.py) and
[`PROTOCOL.md`](../lib/notebooks/PROTOCOL.md).

**Snapshots:**
- Free-tune (frozen coords, no floor): [`results/free_tune_protocol/`](../lib/notebooks/four_lang_cc_bbox_blur/results/free_tune_protocol/)
- Thr-floor (current): [`results/comparison_summary.json`](../lib/notebooks/four_lang_cc_bbox_blur/results/comparison_summary.json)
- Diff: [`results/thr_floor_before_after.json`](../lib/notebooks/four_lang_cc_bbox_blur/results/thr_floor_before_after.json)

All nine cells ran at **thr = 0.95** on CUDA (RTX 5070 Ti).

### Mean defended accuracy — free-tune vs thr-floor
...
### Clean Δ — free-tune vs thr-floor

ΔΔ = thr-floor − free-tune (positive = clean damage improved).
...
### No defense vs defense (thr-floor, new protocol)
...
- **Atk EN** = accuracy of the **English CLIP** on the attacked images, **before** any defense.
- **Atk L** = accuracy of the **partner CLIP** (ZH, KO, or JA for that row) on the same attacked images, **before** defense.
- **Atk mean** = average of Atk EN and Atk L.
- **Def mean** = average of EN and L accuracy **after** `cc_bbox_blur` at thr=0.95.
- **Clean EN / L** = accuracy of EN / partner CLIP on **unattacked** images.
...
- **Floor worked.** Every cell that free-tune had dropped below 0.95 recovered Clean Δ to the same values as other thr=0.95 cells for that partner (ZH −1.5; KO −11.2; JA −11.5).
- **Mean def also rose** where thr had slipped: `zh/multi` 71.7→74.0, `ko/multi` 64.0→69.9, `ja/multi` 73.6→75.9, `ja/uni_en` 66.1→72.3.
- **vs pre-freeze RNG baseline:** ZH multi is 74.0% vs old 74.9% (−0.9pp) with Clean Δ back at −1.5 — essentially restored under frozen coords. Remaining KO/JA Clean Δ gap vs ZH is the known heatmap-quality issue, not thr-tune.
- **Cite thr-floor numbers going forward** as the new-protocol production setting.

**Status:** thr ≥ 0.95 enforced in four_lang + PROTOCOL; recovery run complete.

## 2026-07-22 / 23 — Attack detector (gate `cc_bbox_blur`)

**Question:** Can Attn-last heatmap *shape* separate typographic-attacked images from clean ones, and does gating `cc_bbox_blur` on that detector kill Clean Δ without giving up attacked accuracy?

**Notebook:** [`lib/notebooks/attack_detector/`](../lib/notebooks/attack_detector/) (renamed from `heatmap_attack_detector`). Attack = `multi`, frozen `attack_pos`. Replaces failed one-threshold `gated_peakiness` (`gated_off_frac=0`).

**Shared method (look → learn → use), per partner L:**

1. Bake Attn-last for clean + `multi` attacked (1000 each); 26 scalars from EN / L / intersection (entropy, top-k mass, Gini, kurtosis, cov95, CC count, EN–L corr/IoU).
2. **Phase A:** PCA + t-SNE (evidence that clusters separate).
3. **Phase B:** logistic + calibrated linear SVM; image-level 70/15/15 split; val threshold for attack-recall ≥ **0.99** (raised from 0.95 so gated atk drop stays <1 pp on all partners).
4. **Phase C:** `never_defend` / `always_defend` / `gated` (blur only if detector says attack). Success bar: Clean Δ improves ≥1 pp vs always, attacked mean acc drops ≤1 pp.

Teacher-style dim-reduction = Phase A evidence; the gate is the Phase B decision boundary in feature space (not t-SNE coords).

---

### Step 1 — EN ∩ ZH (`multi`) — 2026-07-22 (retuned 07-23)

Outputs: `results/zh/multi/`.
...
### Step 2 — EN ∩ KO and EN ∩ JA (`multi`) — 2026-07-23

Folder renamed to `attack_detector/`. Same pipeline; separate detector per partner. Outputs: `results/ko/multi/`, `results/ja/multi/`, roll-up `results/comparison_summary.json`.
...
### Combined roll-up (`multi`, recall target 0.99)
...
**Gated defend coverage (full n=1000):**
...
**Retune note:** First pass used attack-recall ≥ 0.95; KO only defended 97.7% of attacks → −1.30 pp atk drop (failed bar). Raising the target to **0.99** lowers the gate threshold, catches nearly all stickers, and brings **all three** partners under 1 pp drop. Tradeoff: KO clean false-blur rate rises to 2.5% (Clean Δ still only −0.20 pp vs always −11.25).

### Verdict

- Heatmap-pattern detection **transfers** to KO and JA: AUC ≈ 1, Clean Δ collapses to ~0 for all three partners.
- Biggest win is KO/JA Clean Δ (**≈ −11 pp → ~0**), where always-defend hurt most.
- After recall-target 0.99 retune, **all three** meet the ≤1 pp attacked-acc drop bar (ZH −0.10, KO −0.45, JA −0.20).
- Caveats: `multi` + frozen sticker geometry only; `uni_en` / `uni_l` and adaptive placement still open.

**Status:** Step 1+2 complete under `attack_detector/` with recall≥0.99; gated defense is the clear Clean-Δ fix for EN∩L `multi`, and all partners pass the success bar.

**Phase A/B figures (2026-07-25):** full embeds + process/ROC panels → section **2026-07-25 — Phase A/B gated occlusion visualization** (end of diary). Script: [`make_phase_ab_viz.py`](../lib/notebooks/attack_detector/make_phase_ab_viz.py). Quick look (ZH):

![ZH process](figures/attack_detector/zh/phase_ab_process.png)

![ZH PCA](figures/attack_detector/zh/pca_features.png)

![ZH ROC/CM](figures/attack_detector/zh/phase_b_roc_cm.png)

---

## 2026-07-23 — Four paper baselines vs `cc_bbox_blur`

Living numbers (always update this first): [`docs/baseline_comparison.md`](baseline_comparison.md).  
Code: [`lib/notebooks/paper_baselines/`](../lib/notebooks/paper_baselines/).  
Protocol: dual-box multi (EN+ZH stickers), frozen n=1000 sample, 224×224. Smoke ladder n=16 → n=100 → n=1000 for each method.

### Leaderboard (final n=1000)

| Method | Acc | Clean Δ | Cost | Scope |
|--------|----:|--------:|-----:|-------|
| **cc_bbox_blur** (ours) | **74.9%** mean | **−1.5pp** | 4 | EN∩ZH |
| OCR + blur | 73.8% mean | −0.7pp | 3 | EN+ZH |
| Defense-Prefix (CIFAR EN+ZH) | 59.2% mean (EN 73.8 / ZH 44.5) | +0.5pp | 2 | ZH port 2026-07-25 |
| Dyslexify-style head ablation | 20.0% EN | 0.0pp | 2 | EN-only |
| SamplingTAR-style circuit ablation | 11.6% EN | +0.2pp | 2 | EN-only |

Vanilla multi attack (same sample): EN ~4.5%, ZH ~6.4%.

### Per-method verdicts

**1. Defense-Prefix (Azuma & Matsui 2023)**  
Published ImageNet `dp_vit-b32.pt` failed Gate A (0/16 preds changed). Retrained 10 epochs on CIFAR-10 **train** (20k, synthetic dual-box typos; eval sample unused). Final EN **73.8%**, Clean Δ **+0.5pp**, ASR **16.4%**. ZH ChineseCLIP port (2026-07-25): **44.5%**, ASR **52.5%** → mean EN+ZH **59.2%**, well below our **74.9%**. EN column still beats our EN 71.6%; ZH DP is not competitive with spatial.

**2. OCR + blur**  
EasyOCR `en`+`ch_sim` → Gaussian blur r=12 → reclassify both models. Detect-img rate 100%; sticker hit **90.3%**. Final mean **73.8%** (EN 72.8 / ZH 74.7), Clean Δ **−0.7pp**. Closest published spatial peer; we win by **+1.1pp** mean, mostly from better ZH recovery (78.2% vs 74.7%) and not missing ~10% of stickers. No external OCR dependency.

**3. Dyslexify (Hufe et al. 2026) — style port**  
Typo-attn score on sticker patches + greedy/prefix head select; CLS←spatial redirect on open_clip ViT-B/32 openai (paper: LAION B/16). Final EN **20.0%**, Clean Δ 0, ASR still 78.3%. Hook validated (preds change); ablation alone is not competitive on this dual-box protocol.

**4. SamplingTAR (Liu et al., ECCV 2026) — style port**  
CLS-attn mass mining (no SAE) + z-threshold (best z=2.0, 7 heads) + `fix_attn`. Final EN **11.6%**, Clean Δ +0.2pp. Same qualitative failure mode as Dyslexify.

### Overall takeaway

- **Spatial defenses dominate** dual-box CIFAR: `cc_bbox_blur` still leads mean (**74.9%**), with OCR+blur a close second (**73.8%**).
- **Prompt defense (DP)** is strong on EN after CIFAR retrain and nearly free on clean images, but leaves high ASR and does not cover ZH — complementary story, not a replacement for EN∩L intersection.
- **Mechanistic head ablations** (Dyslexify / SamplingTAR ports) only recover ~12–20% EN; useful negative baselines for the paper (“attention heads alone ≠ sticker defense”).
...
## 2026-07-25 — ZH Defense-Prefix (ChineseCLIP CIFAR retrain)

**Question:** Can we give Defense-Prefix a fair ZH number (not EN-token-on-ZH), so the baseline leaderboard reports EN **and** ZH?

**Decision:** Port DP to ChineseCLIP only. Skip Dyslexify / SamplingTAR ZH (high hook cost, EN already fails). Skip 4-lang DP.

### Protocol (mirror EN)

| Piece | Detail |
|-------|--------|
| Code | [`train_cifar_dp_zh.py`](../lib/notebooks/paper_baselines/defense_prefix/train_cifar_dp_zh.py), [`zh_dp_encode.py`](../lib/notebooks/paper_baselines/defense_prefix/zh_dp_encode.py), [`run_eval_zh.py`](../lib/notebooks/paper_baselines/defense_prefix/run_eval_zh.py) |
| Backbone | `OFA-Sys/chinese-clip-vit-base-patch16`, **frozen** (CUDA) |
| Trainable | One DP embedding (768-d word-emb space), init `Normal(0, 0.02)` |
| Prompt | `一张*{class}的照片。` — `*` (tok id 115) embedding replaced at encode |
| Data | CIFAR-10 **train** n=20k seed 0; dual-box **ZH** wrong-class typos (msyh); eval 1000 held out |
| Opt / loss | Same as EN: SGD 0.002, cosine→5e-5, 10 ep, batch 128; `CE + 3·KL` |
| Wall time | ~19 min train (RTX 5070 Ti); Gate ladder ~40 s |
| Artifact | [`dp_cifar10_zh_vit-b16.pt`](../lib/notebooks/paper_baselines/defense_prefix/results/dp_cifar10_zh_vit-b16.pt) |

### Results (frozen dual-box multi, n=1000)

| Lang | Defended acc | ASR | Clean Δ | MIXED2000 always |
|------|-------------:|----:|--------:|-----------------:|
...
**Status:** ZH DP done; leaderboard now EN+ZH for Defense-Prefix.

---

## 2026-07-25 — Phase A/B gated occlusion visualization

**Question:** Can we *see* the gated `cc_bbox_blur` Look→Learn path (Phase A clusters + Phase B gate), not only the Phase C tables?

**Backstory:** Detector write-up **2026-07-22 / 23 — Attack detector (gate `cc_bbox_blur`)** already had PCA/t-SNE/importance PNGs under `results/{L}/multi/` but the diary only quoted numbers. This entry embeds those figures and adds process + ROC/confusion panels.

**Script:** [`lib/notebooks/attack_detector/make_phase_ab_viz.py`](../lib/notebooks/attack_detector/make_phase_ab_viz.py) (cache + sklearn; no CLIP bake). Roll-up: [`results/phase_ab_viz_rollups.json`](../lib/notebooks/attack_detector/results/phase_ab_viz_rollups.json).

**See the figures:** open [`docs/figures/attack_detector/gallery.html`](figures/attack_detector/gallery.html) in a browser, or Markdown Preview on [`figures/attack_detector/README.md`](figures/attack_detector/README.md) (`Ctrl+Shift+V`). The editor source view only shows `![...](...)` as text.

### EN ∩ ZH (`multi`)

Phase A: PCA nearest-centroid test acc **99.0%** (explained var sum 0.667). Phase B: **linear SVM**, test AUC **1.000**, thr≈0.473; full-set fire clean **0.4%** / atk **99.8%**.

![ZH PCA](figures/attack_detector/zh/pca_features.png)

![ZH t-SNE](figures/attack_detector/zh/tsne_features.png)

![ZH feature importance](figures/attack_detector/zh/feature_importance.png)
...
Phase A: PCA nearest-centroid test acc **97.3%** (explained var sum 0.798). Phase B: **linear SVM**, test AUC **0.999**, thr≈0.399; full-set fire clean **2.5%** / atk **99.4%**.

![KO PCA](figures/attack_detector/ko/pca_features.png)

![KO t-SNE](figures/attack_detector/ko/tsne_features.png)

![KO feature importance](figures/attack_detector/ko/feature_importance.png)

![KO process](figures/attack_detector/ko/phase_ab_process.png)

![KO ROC/CM](figures/attack_detector/ko/phase_b_roc_cm.png)

### EN ∩ JA (`multi`)

Phase A: PCA nearest-centroid test acc **97.7%** (explained var sum 0.689). Phase B: **linear SVM**, test AUC **1.000**, thr≈0.541; full-set fire clean **0.3%** / atk **99.8%**.

![JA PCA](figures/attack_detector/ja/pca_features.png)

![JA t-SNE](figures/attack_detector/ja/tsne_features.png)

![JA feature importance](figures/attack_detector/ja/feature_importance.png)

![JA process](figures/attack_detector/ja/phase_ab_process.png)

![JA ROC/CM](figures/attack_detector/ja/phase_b_roc_cm.png)

**Status:** Phase A/B viz written next to existing detector artifacts; diary now shows process strips + ROC/CM for all three partners.


---

## 2026-07-25 — EN neglect vs Gaussian blur (beat DP 81.65%)

**Question:** Does hard-neglecting occluded patches beat soft Gaussian blur for English CLIP, and can any method push EN MIXED2000 above Defense-Prefix **81.65%**?

**Code:** [`lib/notebooks/en_neglect_vs_blur/`](../lib/notebooks/en_neglect_vs_blur/) (`helpers.py`, `run_eval.py`). Protocol: frozen `CIFAR10_BALANCED_1000_SAMPLE`, `multi` EN+ZH dual-box, thr≥0.95 `cc_bbox` masks, CUDA.

### What blur is (current production)

Fill = `PIL.ImageFilter.GaussianBlur(radius=12)` on the full RGB image; copy only masked pixels back. Soft occlude: sticker glyphs smash, but the encoder still sees a smeared RGB blob.

### What neglect is (this trial)

Map the same 224 mask → ViT-B/32 7×7 patches; zero patch tokens **after** pos-embed when coverage ≥0.5 (CLS kept). Controls: flat **mean** fill and solid **black**.

### Oracle ceiling (GT sticker boxes, EN atk)
...
| mean | 72.4% | 2.7% |
| **black** | **74.6%** | 2.6% |
| neglect (ViT token zero) | 63.6% | 4.9% |

Token-level neglect is **worse** than blur/black even with perfect boxes — zeroing patches hurts object evidence more than it helps. Black is the best pure-spatial oracle. Ceiling still below the ~77.4% atk needed to clear 81.65% at gated clean≈85.9%, so fill type alone cannot beat DP.

### Always-on + gated (EN∩ZH `cc_bbox`, EN score)

| Method | policy | atk | clean | MIXED2000 | vs DP |
|--------|--------|----:|------:|----------:|------|
| blur | always | 70.0% | 83.7% | 76.85% | below |
| mean | always | 70.5% | 81.9% | 76.20% | below |
| black | always | 72.9% | 82.2% | 77.55% | below |
| neglect | always | 60.7% | 78.3% | 69.50% | below |
| blur | gated | 69.9% | 85.9% | 77.90% | below |
| mean | gated | 70.4% | 85.9% | 78.15% | below |
| **black** | **gated** | **72.9%** | **85.8%** | **79.35%** | below |
| neglect | gated | 60.5% | 85.9% | 73.20% | below |
| Defense-Prefix | always | 73.8% | 89.5% | **81.65%** | bar |

Best spatial-only: **gated black** 79.35% (+1.45pp over gated blur). Pure ViT neglect loses.

### Escalation (plan order)

- `patch_thr` 0.25/0.5/0.75 neglect: all worse (best MIXED 69.50%).
- OCR boxes + neglect (always-on): atk **50.9%**, clean **84.9%**, ASR **24.0%**, MIXED **67.90%** (misses + token damage; logged only — not preferred vs Attn masks).
- **DP + spatial** (CIFAR-retrained DP text on spatially edited images):

| Hybrid | atk | clean | MIXED2000 |
|--------|----:|------:|----------:|
| DP-only (ref) | 73.8% | 89.5% | 81.65% |
| DP + blur | 82.0% | 90.2% | **86.10%** |
| DP + black | 82.8% | 89.1% | **85.95%** |
| **DP + patch-zero pixels** | **84.2%** | **89.1%** | **86.65%** |

### Verdict

- Soft Gaussian blur is **not** the weak link vs DP; ViT token neglect alone is harmful.
- Among spatial fills, **black > blur > mean > neglect** for EN MIXED2000 (gated).
- Bar cleared by stacking: **DP text + spatial patch-zero** → EN MIXED2000 **86.65%** (+5.00pp over DP-only). Also DP+blur / DP+black clear the bar.
- Artifacts: `results/oracle_n1000.json`, `always_n1000.json`, `gated_n1000.json`, `escalate_n1000.json`, `leaderboard.json`.

**Status:** Goal met via DP+spatial hybrid; pure neglect does not beat blur.


---

## 2026-07-25 — Occlusion-only chase of DP EN MIXED2000 81.65% (no DP)

**Question:** Can our occlusion stack alone beat Defense-Prefix’s EN MIXED2000 **81.65%**, without borrowing DP text tokens?

**Preferred localization:** Attn-last EN∩ZH → `cc_bbox` (heatmap). **No OCR in the production line going forward.** OCR∪Attn arms below are logged for completeness only.

**Also allowed here:** fills (black/blur/mean), Phase-C gate, multi-fill conf-drop pick. **No** Defense-Prefix in the winner column.

**Code:** [`lib/notebooks/en_occlusion_beat_dp/run_eval.py`](../lib/notebooks/en_occlusion_beat_dp/run_eval.py). Artifact: [`results/summary_n1000.json`](../lib/notebooks/en_occlusion_beat_dp/results/summary_n1000.json).

### Why fill-only was already insufficient

Gated clean ≈ vanilla EN **85.9%**. Clearing 81.65% then needs EN atk ≳ **77.4%**. Prior oracle black (perfect boxes) only reached **74.6%** atk.

### Attention-only arms (gated, EN, n=1000 multi)

| Arm | atk | ASR | clean | MIXED2000 | vs DP |
|-----|----:|----:|------:|----------:|------|
| gated cc_bbox black | 72.9% | 3.7% | 85.8% | **79.35%** | below |
| gated cc_bbox ensemble (conf-drop) | 72.3% | 1.9% | 85.8% | 79.05% | below |
| Defense-Prefix (bar) | 73.8% | 16.4% | 89.5% | **81.65%** | — |

Ensemble pick counts (cc_bbox, atk): black 438 / mean 298 / blur 262.

**Attention-only best:** gated cc_bbox black **79.35%** (−2.30pp vs DP).

### OCR-assisted arms (logged; not preferred)

EasyOCR (`en`+`ch_sim`) boxes **union** Attn `cc_bbox`, then same gated fills / ensemble.

| Arm | atk | ASR | clean | MIXED2000 | vs DP |
|-----|----:|----:|------:|----------:|------|
| gated OCR∪cc_bbox black | 73.6% | 3.1% | 85.8% | **79.70%** | below |
| gated OCR∪cc_bbox ensemble (conf-drop) | 73.0% | 1.9% | 85.8% | 79.40% | below |
| gated OCR∪cc_bbox ensemble + raw hyp. | 73.0% | 1.9% | 85.8% | 79.40% | below |

Ensemble pick counts (union, atk): black 433 / mean 311 / blur 254; with raw hyp.: black 420 / mean 302 / blur 234 / raw 42. Gate fire: atk 99.8% / clean 0.4%.

**OCR-assisted best:** gated OCR∪cc_bbox black **79.70%** (+0.35pp over attention-only black, still **−1.95pp** vs DP).

### Related OCR number (earlier neglect escalate)

From [`en_neglect_vs_blur/results/escalate_n1000.json`](../lib/notebooks/en_neglect_vs_blur/results/escalate_n1000.json): OCR boxes + ViT neglect → atk **50.9%**, clean **84.9%**, ASR **24.0%**, MIXED2000 **67.90%** (worse than heatmap black).

### Verdict

- **Did not beat DP** with occlusion only (attention or OCR).
- Preferred attention-only max: **79.35%**. OCR∪Attn black max: **79.70%** (small lift; still short).
- Conf-drop multi-fill ensemble does not beat always-black.
- Ceiling: best atk **73.6%** (OCR∪) / **72.9%** (attn) ≪ **77.4%** needed at clean≈85.9%.

**Status:** Numbers logged (incl. OCR). Production path stays Attn heatmap; DP bar **81.65%** unmet by occlusion alone.


## 2026-07-25 — Dyslexify / SamplingTAR hybrids (clear ~50% EN + MIXED2000)

**Problem:** Heads-only ports were unusable as peer baselines — Dyslexify EN **20.0%** / MIXED2000 **52.95%**, SamplingTAR EN **11.6%** / MIXED2000 **48.85%** — far below OCR (~74%) and DP EN (73.8%).

**Hybrid:** Keep each method’s typo-head mining + selection, then blur patches with high mean CLS→patch attention under those heads (`score ≥ 0.35·max`, cap `top_k=4`, Gaussian r=12), then classify with head ablation still on. No GT/OCR boxes. Code: [`_common/hybrid_spatial.py`](../lib/notebooks/paper_baselines/_common/hybrid_spatial.py), `--mode hybrid` in `dyslexify/run_eval.py` and `sampling_tar/run_eval.py`.

### Final n=1000 (EN multi)

| Method | mode | EN atk | ASR | Clean Δ | MIXED2000 always |
|--------|------|-------:|----:|--------:|-----------------:|
| Dyslexify | heads | 20.0% | 78.3% | 0.0pp | 52.95% |
| Dyslexify | **hybrid** | **66.9%** | 3.7% | −8.1pp | **72.35%** |
| SamplingTAR | heads | 11.6% | 87.5% | +0.2pp | 48.85% |
| SamplingTAR | **hybrid** | **67.3%** | 5.1% | −8.3pp | **72.45%** |
