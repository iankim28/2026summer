...
**Numbers:** Locked to [`paper_tables_final.md`](paper_tables_final.md) / [`tables_index.md`](tables_index.md). MIXED2000 = `0.5 × attacked accuracy + 0.5 × clean-policy accuracy` over the same 1000 images. Values marked † in Table 1 are provisional smoke (n = 100) and are labeled as such in the prose.  
**Last synced:** 2026-08-05 — Abstract–Conclusion rewritten to match Tables 1–4 (Clean Δ EN, bilingual means, OCR 4-lang, Defense-Prefix ZH finals).
...
All baselines used the same frozen stickers on the same 1000 images. Where Table 1 marks a cell with †, that score is from a provisional smoke run (n = 100) rather than a full n = 1000 final.

**4×4 grid occlusion.** The image was partitioned into 16 equal patches. One or two patches were covered greedily by selecting the cover that most reduced the model’s pre-defense top-1 confidence, after which the image was reclassified. Approximate cost was 62 passes. English and Chinese attacked accuracies were measured at n = 1000; Korean, Japanese, mixed scores, and Clean Δ EN are provisional (†). This baseline provides a model-agnostic search floor without attention or text detection.
...
**Defense-Prefix (4).** A prefix token was trained on CIFAR-10 training images with the same sticker style, with CLIP frozen and evaluation images held out. English and Chinese tokens were trained to n = 1000 finals (Chinese for 20 epochs under an EN+ZH multi-sticker schedule). Korean and Japanese scores in Table 1 are provisional (†). Cost was 2.

**Dyslexify (5) and SamplingTAR (6) hybrids.** Typographic heads or circuits were identified and suppressed, then combined with attention-guided patch blur. Cost was 3. English cells are n = 1000 finals; partner-language cells in Table 1 are provisional (†). Head-only variants were retained as negative results. Both implementations follow the style of the original papers and are not reruns of the authors’ exact checkpoints.
...
Scope differs across methods and must be read with the scores. Raw CLIP and OCR + blur report four-language finals. Ours rows are bilingual EN∩L pairings. Grid occlusion, Defense-Prefix Korean/Japanese, and the head hybrids’ partner cells are marked † where provisional.

Grid occlusion recovered a mean attacked accuracy of 49.0%† (47.8% English, 49.2% Chinese at n = 1000) at approximately 62 passes per image. OCR + blur reached 78.8% mean attacked accuracy and 84.0% mean MIXED2000 across four languages at cost 3, with Clean Δ EN of −0.6 pp. Defense-Prefix reached 73.8% on English and 81.4% on Chinese (MIXED2000 81.7% and 86.2%); Korean and Japanese cells were 66.0%† and 88.0%†. The four-language Defense-Prefix means in Table 1 (77.3%† attacked, 83.6%† MIXED2000) therefore mix finals with smoke. The SamplingTAR and Dyslexify hybrids reached 67.3% and 66.9% English attacked accuracy, with Clean Δ EN of −8.3 and −8.1 pp.
...
Scoring grid patches by confidence drop rather than raw confidence raised mean attacked accuracy from approximately 11% to the Table 1 grid floor (49.0%†), indicating that the scoring rule, rather than search alone, accounted for most of the gain. Exhaustive search over all 120 patch pairs did not close the remaining gap, because a sticker straddling a patch boundary cannot be covered cleanly by a 1/16 tile. Patch-hit analysis showed that covering the English box contributed more than covering the partner box.
...
Comparisons with other defenses should be interpreted with scope. Grid occlusion provides a weak floor (49.0%† mean attacked accuracy at ~62 passes). OCR + blur is the strongest four-language mean in Table 1 (78.8% attacked, 84.0% MIXED2000) when recognition succeeds, and gated black approaches the English–Chinese OCR average without OCR. Defense-Prefix retains a higher English MIXED2000 (81.7% versus 79.4%), so occlusion alone does not surpass a learned English prefix on English; Chinese Defense-Prefix is also strong (81.4% / 86.2%). Head-only Dyslexify and SamplingTAR interventions were ineffective on dual-box attacks; hybrids that added blur recovered partially (English 67.3% and 66.9%) but incurred large clean costs (−8.3 and −8.1 pp) and did not match gated black. Spatial occlusion, rather than head ablation, accounted for most of the recovery.
